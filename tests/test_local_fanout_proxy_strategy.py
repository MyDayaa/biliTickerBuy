...
def _url_with_decoded_path(url: str) -> str:
...
def test_h2_client_constructor_uses_abstract_client_interface():
    ...
    assert _url_with_decoded_path(client.calls[1][1]) == url
    assert client.calls[1][2:] == (None, {"project_id": 1})
    assert client.calls[2][0] == "get"
    assert _url_with_decoded_path(client.calls[2][1]) == url
...
def test_proxy_pool_fanout_builds_one_create_connection_per_proxy():
    ...
    assert 1 <= len(business_connections) <= 2
    assert {instance.proxy_url for instance in business_connections}.issubset(
        {
            "http://127.0.0.1:18080",
            "socks5://127.0.0.1:19090",
        }
